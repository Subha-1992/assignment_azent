...
with open('C:/Users/subha.malakar/Downloads/arizona-state-university-s1.json') as f:
    s1 = json.load(f)
with open('C:/Users/subha.malakar/Downloads/arizona-state-university-s2.json') as f:
    s2 = json.load(f)
...
for i in range(len(s1['programs'])):
    highest = process.extractOne(s1['programs'][i]['name'], course_name,scorer=fuzz.token_set_ratio)
    if highest[1]>90:
...
#Remove similar programs from S2
index_in_s2_set=list(set(index_in_s2))
index_in_s2_set.sort(reverse=True)
for i in index_in_s2_set:
    s2['programs'].pop(i)
...
with open("C:/Users/subha.malakar/Downloads/arizona-state-university-s1_s2.json", "w") as outfile:
